    ...
    try {
        ...
        if (provider.id === 'Mock') {
            ...
            const existingConnection = await db
                .select()
                .from(wearableConnections)
                .where(and(eq(wearableConnections.userId, userId), eq(wearableConnections.provider, provider.id)))
                .limit(1);
    ...
    try {
        ...
        if ('revoke' in provider && typeof provider.revoke === 'function') {
            ...
            await db
                ...
                .where(and(eq(wearableConnections.userId, userId), eq(wearableConnections.provider, providerName)));
        }
        return res.json({ success: true });
    }
    catch (error) {
...
router.post('/health/sync', requireAuth, async (req, res) => {
    try {
        ...
        const existingReport = await db
            .select()
            .from(healthReports)
            .where(and(eq(healthReports.userId, userId), eq(healthReports.date, today)))
            .limit(1);
      ...
      `);
        ...
        const last14Workouts = await db
            .select()
            .from(workouts)
            .where(and(eq(workouts.userId, userId), gte(workouts.createdAt, fourteenDaysAgo)))
        ...
        try {
            ...
            if (providerName) {
                await db
                    ...
                    .where(and(eq(wearableConnections.userId, req.user.id), eq(wearableConnections.provider, providerName)));
            }
        }
        catch (updateError) {
...
router.get('/health/reports', requireAuth, async (req, res) => {
    try {
        ...
        const reports = await db
            .select()
            .from(healthReports)
            .where(and(eq(healthReports.userId, userId)))
...
router.post('/health/compute/daily', requireAuth, async (req, res) => {
    try {
        ...
        const existingReport = await db
            .select()
            .from(healthReports)
            .where(and(eq(healthReports.userId, userId), eq(healthReports.date, targetDate)))
            .limit(1);
        const reportData = {
