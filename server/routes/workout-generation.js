...
import { isWorkoutV2Enabled, shouldShowMetricsDebug } from "../config/flags.js";
import { generateWorkoutPlan } from "../workouts/engine.js";
// Telemetry for RL training data collection
import { logGenerationEvent, extractMetricsSnapshot } from "../workouts/telemetry.js";
...
async function composeUserContext(userId) {
    try {
        ...
        const yesterdayWorkout = recentWorkouts.find(w => w?.createdAt && w.createdAt.toISOString().split('T')[0] === yesterdayStr);
        // Count weekly categories
        const weekAgo = new Date();
        weekAgo.setDate(weekAgo.getDate() - 7);
        const weeklyWorkouts = recentWorkouts.filter(w => w?.createdAt && w.createdAt >= weekAgo);
        const weeklyCounts = {};
...
async function convertToV2Format(userId, context, request) {
    ...
    const history = context?.yesterday ? [{
            ...
            primaryPattern: 'squat', // Simplified mapping
 ...
 */
export function registerWorkoutGenerationRoutes(app) {
    ...
    app.post("/api/generate/v2", requireAuth, async (req, res) => {
        ...
        try {
            ...
            const workoutPlan = generateWorkoutPlan(workoutRequest, history, [], // No progression states yet - could be loaded from database
            biometrics, debugInfo.energySystemsHistory || undefined);
    ...
    if (process.env.NODE_ENV !== 'production') {
        app.post("/api/dev/generate/v2", async (req, res) => {
            try {
                ...
                // Generate workout using V2 engine
                const workoutPlan = generateWorkoutPlan(workoutRequest, history, [], // No progression states
                biometrics, debugInfo.energySystemsHistory || undefined);
