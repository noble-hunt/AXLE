 ...
 */
export function registerSuggestionRoutes(app) {
    /**
    ...
    app.get("/api/suggestions/today", async (req, res) => {
        ...
        try {
            ...
            try {
                const columnCheck = await db.execute(sql `SELECT column_name FROM information_schema.columns 
        ...
        try {
            ...
            const workoutData = {
                ...
                workout: {
                    ...
                    sets: generatedWorkout.blocks?.map((block, index) => ({
                        id: `block-${index}`,
