...
export function loadBlocks() {
    ...
    const blockData = {
        warmup: warmupBlocksRaw,
        primary: primaryBlocksRaw,
        accessory: accessoryBlocksRaw,
        conditioning: conditioningBlocksRaw,
        finisher: finisherBlocksRaw,
        cooldown: cooldownBlocksRaw,
    };
        ...
        for (let i = 0; i < rawBlocks.length; i++) {
            try {
                ...
                if (validationError instanceof z.ZodError) {
                    errors.push(`${category}.json[${i}]: ${validationError.issues.map(issue => `${issue.path.join('.')}: ${issue.message}`).join(', ')}`);
                }
                else {
 ...
 */
export function initializeBlockLibrary() {
 ...
 */
export function getBlocks(filter) {
    if (!isLoaded) {
    ...
    return blocksCache.filter(block => {
        ...
        if (filter.equipment) {
            const hasRequiredEquipment = block.minEquipment.every(required => filter.equipment.includes(required));
        ...
        if (filter.contraindications && filter.contraindications.length > 0) {
            const hasContraindication = block.contraindications.some(contraindication => filter.contraindications.includes(contraindication));
