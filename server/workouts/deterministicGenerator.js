 ...
 */
export function generateDeterministicWorkout(request) {
    ...
    const exercises = selectedBlocks.flatMap(block => convertBlockToExercises(block, intensity, rng));
 ...
 * Determine workout structure based on duration and goal
 */
function determineWorkoutStructure(durationMin, goal, rng) {
    const structures = {
        'Strength': {
 ...
 */
function convertBlockToExercises(block, intensity, rng) {
 ...
 */
function determineSetsReps(blockType, intensity, rng) {
    const baseRanges = {
 ...
 */
function generateExerciseNotes(movement, blockType, rng) {
    const generalNotes = [
 ...
 */
function generateWorkoutTitle(goal, durationMin, intensity, rng) {
