 ...
 */
export function filterByEquipment(movements, availableEquipment) {
    return movements.filter(movement => movement.equipment.some(eq => availableEquipment.includes(eq)));
}
/**
 * Filter out movements that violate user constraints
 */
export function avoidConstraints(movements, constraints) {
    const constraintMap = {
    ...
    return movements.filter(movement => constraints.every(constraint => {
 ...
 */
export function sampleBalanced(pool, count, rng, options = {}) {
    ...
    const patterns = [
        ...preferredPatterns,
        ...Object.keys(patternCounts).filter(p => !preferredPatterns.includes(p))
    ];
    ...
    for (let i = 0; i < count && availablePool.length > 0; i++) {
        ...
        let candidates = availablePool.filter(m => patternCounts[m.pattern] < maxPerPattern &&
 ...
 */
export function getMovementsByEnergySystem(energySystem) {
    return MOVEMENT_LIBRARY.filter(m => m.energySystem === energySystem);
}
/**
 * Get movements by complexity level
 */
export function getMovementsByComplexity(maxComplexity) {
