 ...
 */
import { MOVEMENT_LIBRARY, filterByEquipment, avoidConstraints, sampleBalanced, createSeededRandom } from './movementTaxonomy.js';
import { selectTemplate } from './templates.js';
import { getIntensityParameters, applyHealthCaps } from './intensity.js';
import { generateProgressionDirectives, convertToWorkoutHistory } from './progression.js';
import { generateWarmup, generateCooldown, getRecommendedWarmupDuration, needsExtendedCooldown } from './warmupCooldown.js';
/**
 * Main workout generation function
 */
export async function generateWorkout(seed, workoutHistory = []) {
    ...
    const adjustedIntensity = Math.max(1, Math.min(10, cappedIntensity + progressionDirectives.intensityAdjustment));
    // Select workout template
    const template = selectTemplate(seed.inputs.archetype, seed.inputs.minutes, adjustedIntensity, rng);
    if (!template) {
    ...
    const availableMovements = filterMovementPool(MOVEMENT_LIBRARY, seed.inputs.equipment, seed.inputs.constraints || [], intensityParams.complexityLimit);
    ...
    const warmupPlan = generateWarmup(mainPatterns, seed.inputs.equipment, warmupMinutes, seed.rngSeed);
    ...
    for (const templateBlock of template.blocks.filter(b => b.type !== 'warmup' && b.type !== 'cooldown')) {
        const workoutBlock = await generateWorkoutBlock(templateBlock, availableMovements, intensityParams, progressionDirectives, rng);
    ...
    if (needsExtendedCooldown(adjustedIntensity)) {
        const cooldownPlan = generateCooldown(adjustedIntensity, mainPatterns, seed.inputs.equipment, seed.rngSeed);
 ...
 */
async function generateWorkoutBlock(templateBlock, availableMovements, intensityParams, progressionDirectives, rng) {
    const exercises = [];
    for (const slot of templateBlock.movements) {
        // Filter movements for this slot
        let slotMovements = availableMovements.filter(m => slot.patterns.includes(m.pattern));
        ...
        if (selectedMovements.length === 0) {
            ...
            const fallbackMovements = availableMovements.filter(m => slot.patterns.includes(m.pattern));
            if (fallbackMovements.length > 0) {
                ...
                for (const movement of fallbackSelected) {
                    const exercise = createExerciseFromMovement(movement, templateBlock, intensityParams, progressionDirectives, slot.role);
 ...
 */
export async function generateDeterministicWorkout(inputs, context, rngSeed, workoutHistory = []) {
    const seed = {
        rngSeed,
        inputs,
        context,
...
function validateWorkoutStructure(workout) {
    if (!workout.id)
        throw new Error('Workout missing required field: id');
    if (!workout.name)
        throw new Error('Workout missing required field: name');
    if (!workout.description)
        throw new Error('Workout missing required field: description');
    if (!workout.totalMinutes || workout.totalMinutes <= 0)
        throw new Error('Workout missing valid totalMinutes');
    if (!workout.estimatedIntensity || workout.estimatedIntensity <= 0)
        throw new Error('Workout missing valid estimatedIntensity');
    if (!workout.blocks || workout.blocks.length === 0)
        throw new Error('Workout missing blocks');
    if (!workout.coaching_notes)
        throw new Error('Workout missing coaching_notes');
    if (!workout.metadata)
        throw new Error('Workout missing metadata');
    // Validate each block
    workout.blocks.forEach((block, index) => {
        if (!block.id)
            throw new Error(`Block ${index} missing required field: id`);
        if (!block.name)
            throw new Error(`Block ${index} missing required field: name`);
        if (!block.type)
            throw new Error(`Block ${index} missing required field: type`);
        if (!block.exercises || block.exercises.length === 0)
            throw new Error(`Block ${index} missing exercises`);
        // Validate each exercise
        block.exercises.forEach((exercise, exerciseIndex) => {
            if (!exercise.id)
                throw new Error(`Block ${index} exercise ${exerciseIndex} missing required field: id`);
            if (!exercise.name)
                throw new Error(`Block ${index} exercise ${exerciseIndex} missing required field: name`);
            if (!exercise.movement)
                throw new Error(`Block ${index} exercise ${exerciseIndex} missing required field: movement`);
            if (!exercise.sets || exercise.sets <= 0)
                throw new Error(`Block ${index} exercise ${exerciseIndex} missing valid sets`);
            if (!exercise.reps)
                throw new Error(`Block ${index} exercise ${exerciseIndex} missing required field: reps`);
        });
    });
}
