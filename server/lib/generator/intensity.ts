...
export interface IntensityParameters {
  ...
  timeUnderTension: number; // seconds per set
  loadPercentage: [number, number]; // min-max %1RM range
  complexityLimit: number; // 1-5 movement complexity
  volumeMultiplier: number; // 0.5-1.5
}

export interface SessionIntensityPlan {
  phases: IntensityPhase[];
  peakPhase: number; // index of peak intensity phase
  avgIntensity: number;
}
 ...
 */
const INTENSITY_MAP: Record<number, IntensityParameters> = {
  1: {
    totalSets: 8,
    avgRestSeconds: 60,
    timeUnderTension: 20,
 ...
 */
export function applyHealthCaps(
  targetIntensity: number,
  axleMetrics: AxleMetrics
): number {
 ...
 */
export function adjustVolumeForPerformance(
  baseParameters: IntensityParameters,
  axleMetrics: AxleMetrics
): IntensityParameters {
 ...
 */
export function shouldShortenSession(axleMetrics: AxleMetrics): boolean {
 ...
 */
export function createSessionIntensityPlan(
  targetIntensity: number,
  sessionMinutes: number,
  axleMetrics: AxleMetrics = {}
): SessionIntensityPlan {
  const cappedIntensity = applyHealthCaps(targetIntensity, axleMetrics);
  ...
  for (let i = 0; i < numPhases; i++) {
    ...
    if (numPhases <= 3) {
      ...
      if (i === 0) phaseIntensity = Math.max(1, cappedIntensity - 2);
      else if (i === 1) phaseIntensity = cappedIntensity;
      else phaseIntensity = Math.max(1, cappedIntensity - 1);
    } else {
      // Multi-phase gentle wave
      const progress = i / (numPhases - 1);
      const waveValue = Math.sin(progress * Math.PI);
      phaseIntensity = Math.round(
        Math.max(1, cappedIntensity - 2) + 
        waveValue * 2
      );
    }
 ...
 */
function getPhaseDescription(phaseIndex: number, totalPhases: number, intensity: number): string {
  if (phaseIndex === 0) return "Warm-up and activation";
  if (phaseIndex === totalPhases - 1) return "Cool-down and recovery";
  
  if (intensity <= 3) return "Easy preparation";
  if (intensity <= 5) return "Moderate work";
  if (intensity <= 7) return "Challenging effort";
  if (intensity <= 9) return "High intensity";
 ...
 */
export function needsExtendedCooldown(intensity: number): boolean {
  return intensity >= 7;
}
...
export function getRestRecommendation(
  intensity: number,
  exerciseType: 'strength' | 'conditioning' | 'skill'
): number {
 ...
 */
export function getHeartRateZones(intensity: number, maxHR?: number) {
  if (!maxHR) maxHR = 190; // Default estimate
