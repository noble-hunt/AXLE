 ...
 */
import { filterByEquipment, createSeededRandom } from './movementTaxonomy.js';
/**
 ...
 */
export function generateWarmup(mainWorkoutPatterns, availableEquipment, targetMinutes, rngSeed) {
    ...
    const generalMobility = availableWarmups.filter(m => ['cat_cow', 'hip_circle', 'arm_circle'].includes(m.id));
    ...
    for (const pattern of patternPriority.slice(0, 3)) { // Max 3 patterns
        const patternWarmups = availableWarmups.filter(m => m.pattern === pattern && !exercises.some(e => e.movement.id === m.id));
    ...
    if (targetMinutes >= 8) {
        const activationMoves = availableWarmups.filter(m => ['glute_bridge', 'scap_pushup', 'band_pull_apart'].includes(m.id) &&
    ...
    if (targetMinutes >= 10) {
        const dynamicMoves = availableWarmups.filter(m => ['inchworm', 'world_greatest_stretch'].includes(m.id) &&
            !exercises.some(e => e.movement.id === m.id));
 ...
 */
export function generateCooldown(workoutIntensity, mainWorkoutPatterns, availableEquipment, rngSeed) {
    ...
    for (const pattern of stretchPatterns.slice(0, 2)) {
        const patternStretches = availableCooldowns.filter(m => (m.pattern === pattern || (pattern === 'hinge' && m.pattern === 'rotation')) &&
            !exercises.some(e => e.movement.id === m.id));
    ...
    const restorativeMoves = availableCooldowns.filter(m => ['childs_pose', 'legs_up_wall', 'savasana'].includes(m.id) &&
        !exercises.some(e => e.movement.id === m.id));
 ...
 */
function generateWarmupDescription(patterns, exerciseCount) {
 ...
 */
export function getRecommendedWarmupDuration(workoutIntensity, workoutType) {
    const baseMinutes = {
        'strength': 8,
        'conditioning': 6,
        'endurance': 5,
 ...
 */
export function needsExtendedCooldown(workoutIntensity) {
