...
import type { MovementPattern, EnergySystem } from './movementTaxonomy.js';

export type WorkoutArchetype = 'strength' | 'conditioning' | 'endurance' | 'mixed';
export type BlockType = 'warmup' | 'main' | 'accessory' | 'conditioning' | 'cooldown';
export type SetStructure = 'straight' | 'superset' | 'circuit' | 'emom' | 'amrap' | 'tabata' | 'intervals';

export interface MovementSlot {
  role: string; // 'primary', 'secondary', 'accessory', 'conditioning'
...
export interface WorkoutBlock {
  ...
  sets?: number;
  reps?: number | string; // e.g., "8-12" or "max"
  time?: number; // minutes
  rest?: number; // seconds
  load?: string; // e.g., "75-85%", "moderate", "bodyweight"
...
export interface WorkoutTemplate {
  ...
  minMinutes: number;
  maxMinutes: number;
  targetIntensityRange: [number, number]; // 1-10 scale
  blocks: WorkoutBlock[];
  description?: string;
}
 ...
 */
const QUICK_STRENGTH_TEMPLATES: WorkoutTemplate[] = [
  {
 ...
 */
const MIXED_TEMPLATES: WorkoutTemplate[] = [
  {
    id: 'mixed-emom-18',
    name: 'Mixed EMOM 18',
    archetype: 'mixed',
 ...
 */
const ENDURANCE_TEMPLATES: WorkoutTemplate[] = [
  {
 ...
 */
export function getTemplatesByArchetype(archetype: WorkoutArchetype): WorkoutTemplate[] {
 ...
 */
export function getTemplatesByDuration(minutes: number): WorkoutTemplate[] {
...
export function selectTemplate(
  ...
  let candidates = WORKOUT_TEMPLATES.filter(t => 
  ...
  if (candidates.length === 0) {
    if (archetype === 'conditioning') {
      candidates = WORKOUT_TEMPLATES.filter(t => 
        (t.archetype === 'mixed' || t.archetype === 'endurance') &&
        minutes >= t.minMinutes && minutes <= t.maxMinutes
      );
    }
  }
  ...
  const intensityMatches = candidates.filter(t => 
    targetIntensity >= t.targetIntensityRange[0] && 
 ...
 */
export function estimateWorkoutTime(template: WorkoutTemplate): number {
