 ...
 */
export function winsorize(values, lowerPercentile = 0.05, upperPercentile = 0.95) {
    if (values.length === 0)
        return [];
 ...
 */
export function computeRollingBaseline(values) {
    if (values.length === 0) {
 ...
 */
export async function computeBaselines(userId, windowDays = 21) {
    ...
    try {
        ...
        (reports || []).forEach(report => {
            if (report.metrics && typeof report.metrics === 'object') {
                ...
                if (typeof metrics.hrv === 'number' && !isNaN(metrics.hrv))
                    hrvValues.push(metrics.hrv);
                if (typeof metrics.restingHR === 'number' && !isNaN(metrics.restingHR))
                    restingHRValues.push(metrics.restingHR);
                if (typeof metrics.sleepScore === 'number' && !isNaN(metrics.sleepScore))
                    sleepScoreValues.push(metrics.sleepScore);
                if (typeof metrics.stress === 'number' && !isNaN(metrics.stress))
                    stressValues.push(metrics.stress);
                if (typeof metrics.steps === 'number' && !isNaN(metrics.steps))
                    stepsValues.push(metrics.steps);
            }
        });
        return {
...
function scoreVitality(sleepScore, steps, hrv, restingHR, stress, baselines) {
    ...
    return Math.round(sleepComponent * 0.4 +
 ...
 */
function scorePerformancePotential({ hrv, sleepScore, strain48h, rpe24h }) {
    let score = 100; // Start optimistic
    // HRV penalty (if low)
    if (hrv != null && isFinite(hrv)) {
        if (hrv < 20)
            score -= 30; // Very low HRV
        else if (hrv < 30)
            score -= 15; // Moderately low HRV
        else if (hrv > 50)
            score += 5; // High HRV bonus
    }
    // Sleep penalty
    if (sleepScore != null && isFinite(sleepScore)) {
        if (sleepScore < 60)
 ...
 */
function scoreCircadian({ sleepMidpointSd, wakeTime, sunrise, stepsFirst2h, uvMax }) {
    ...
    if (wakeTime && sunrise) {
        try {
            ...
            if (diff <= 1)
                ...
                alignmentScore = Math.max(20, 60 - (diff * 10));
    ...
    if (uvMax != null && isFinite(uvMax)) {
        ...
        if (uvMax >= 3 && uvMax <= 7)
            ...
            uvScore = Math.max(30, 80 - ((uvMax - 7) * 10));
        else
            uvScore = Math.max(20, uvMax * 25);
        score += uvScore;
        componentCount++;
    }
...
function scoreEnergyBalance(zoneMinutes14d) {
    ...
    if (easyPct >= 70 && easyPct <= 80)
        score += 25;
    else if (easyPct >= 60 && easyPct <= 85)
        score += 15;
    else if (easyPct >= 50 && easyPct <= 90)
        score += 5;
    else
        score -= Math.abs(easyPct - 75) * 0.5;
    // Hard zone scoring (optimal 10-20%)
    if (hardPct >= 10 && hardPct <= 20)
        score += 25;
    else if (hardPct >= 5 && hardPct <= 25)
        score += 15;
    else if (hardPct >= 0 && hardPct <= 30)
        score += 5;
    else
        score -= Math.abs(hardPct - 15) * 0.5;
    return clamp(score, 0, 100);
}
// ============================================================================
 ...
 */
export async function computeDailyMetrics(userId, date, location) {
    try {
        ...
        const environment = await getEnvironment(location?.lat ?? null, location?.lon ?? null, date);
        // 4. Compute baselines
        const baselines = await computeBaselines(userId);
        // 5. Calculate all scores
        const vitalityScore = scoreVitality(rawBiometrics.sleepScore ?? null, rawBiometrics.steps ?? null, rawBiometrics.hrv ?? null, rawBiometrics.restingHR ?? null, rawBiometrics.stress ?? null, baselines);
 ...
 */
export function runMetricsUnitTests() {
...
export { scoreSleep, scoreActivity, scoreStressRecovery, scoreVitality, scorePerformancePotential, scoreCircadian, scoreEnergyBalance };
