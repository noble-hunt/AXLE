...
// Types for the environment service
export interface SolarData {
  sunrise: string | null;
  sunset: string | null;
  dayLength: number | null; // in hours
}
...
export interface WeatherData {
  ...
  precipitationProbability: number | null; // percentage
  uvIndex: number | null;
}
...
class EnvironmentCache {
  ...
  private generateKey(lat: number, lon: number, date: string, type: string): string {
    return `${lat.toFixed(4)}|${lon.toFixed(4)}|${date}|${type}`;
  }

  get(lat: number, lon: number, date: string, type: string): any | null {
  ...
  set(lat: number, lon: number, date: string, type: string, data: any): void {
    const key = this.generateKey(lat, lon, date, type);
    const now = Date.now();
 ...
 */
export async function getSolar(lat: number | null | undefined, lon: number | null | undefined, date: Date | string): Promise<SolarData> {
  const nullResult: SolarData = {
    sunrise: null,
    sunset: null,
    dayLength: null
  };
 ...
 */
export async function getWeather(lat: number | null | undefined, lon: number | null | undefined, date: Date | string): Promise<WeatherData> {
  const nullResult: WeatherData = {
    temperature: null,
    humidity: null,
    windSpeed: null,
 ...
 */
export async function getAQI(lat: number | null | undefined, lon: number | null | undefined, date: Date | string): Promise<AQIData> {
  const nullResult: AQIData = {
    pm25: null,
    pm10: null,
    no2: null,
    o3: null,
  ...
  try {
    ...
    // Calculate averages
    const getAverage = (param: string): number | null => {
    ...
    if (pm25 !== null) {
      ...
      if (pm25 <= 12) overallIndex = 1; // Good
      else if (pm25 <= 35) overallIndex = 2; // Moderate
      else if (pm25 <= 55) overallIndex = 3; // Unhealthy for sensitive
      else if (pm25 <= 150) overallIndex = 4; // Unhealthy
      else if (pm25 <= 250) overallIndex = 5; // Very unhealthy
      else overallIndex = 6; // Hazardous
    }
 ...
 */
export async function getEnvironment(lat: number | null | undefined, lon: number | null | undefined, date: Date | string): Promise<EnvironmentData> {
  const dateStr = typeof date === 'string' ? date : format(date, 'yyyy-MM-dd');
