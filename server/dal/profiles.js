...
export async function updateProfileProviders(userId, provider) {
    ...
    try {
        ...
        if (error) {
            ...
            if (error.message?.includes('0 rows') || error.code === 'PGRST116') {
                ...
                const insertCols = ['user_id', ...Object.keys(updates).map(k => {
                        ...
                        if (k === 'favoriteMovements')
                            return 'favorite_movements';
                        return k;
                    })];
        ...
        try {
            ...
            if (result.rowCount === 0) {
                ...
                const insertVals = [userId];
                let idx = 1;
                if (updates.firstName !== undefined) {
                    insertCols.push('first_name');
                    insertVals.push(updates.firstName);
                }
                if (updates.lastName !== undefined) {
                    insertCols.push('last_name');
                    insertVals.push(updates.lastName);
                }
                if (updates.username !== undefined) {
                    insertCols.push('username');
                    insertVals.push(updates.username);
                }
                if (updates.dateOfBirth !== undefined) {
                    insertCols.push('date_of_birth');
                    insertVals.push(updates.dateOfBirth);
                }
                if (updates.avatarUrl !== undefined) {
                    insertCols.push('avatar_url');
                    insertVals.push(updates.avatarUrl);
                }
                if (updates.preferredUnit !== undefined) {
                    insertCols.push('preferred_unit');
                    insertVals.push(updates.preferredUnit);
                }
                if (updates.favoriteMovements !== undefined) {
                    insertCols.push('favorite_movements');
                    insertVals.push(updates.favoriteMovements);
                }
                const placeholders = insertVals.map((_, i) => `$${i + 1}`).join(', ');
