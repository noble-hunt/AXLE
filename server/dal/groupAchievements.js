...
export async function computeSquadGoalsProgress(groupId) {
    ...
    const result = await db
        ...
        .where(and(eq(groupPosts.groupId, groupId), gte(posts.createdAt, oneWeekAgo)));
...
export async function computeHypeTrainProgress(groupId) {
    const oneWeekAgo = new Date();
    oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
    const result = await db
        .select({ count: count() })
        .from(groupReactions)
        .where(and(eq(groupReactions.groupId, groupId), gte(groupReactions.createdAt, oneWeekAgo)));
...
export async function computePrPartyProgress(groupId) {
    ...
    const result = await db
        ...
        .from(groupPosts)
        .innerJoin(posts, eq(groupPosts.postId, posts.id))
        .where(and(eq(groupPosts.groupId, groupId), eq(posts.kind, "pr"), gte(posts.createdAt, oneMonthAgo)));
...
export async function computeGroupAchievements(groupId) {
...
export async function upsertGroupAchievements(groupId, achievements) {
...
export async function recomputeAndUpdateGroupAchievements(groupId) {
    // Get current achievements
    const currentAchievements = await getGroupAchievements(groupId);
    const currentUnlocked = new Set(currentAchievements.filter(a => a.unlocked).map(a => a.name));
    // Compute new achievements
    const newAchievements = await computeGroupAchievements(groupId);
    // Find newly unlocked achievements
    const newlyUnlocked = newAchievements.filter(achievement => achievement.unlocked && !currentUnlocked.has(achievement.name));
