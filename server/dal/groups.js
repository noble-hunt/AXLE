...
export async function createGroup(userId, params) {
    const database = db;
...
export async function deleteGroupPost(userId, groupId, postId) {
    const database = db;
    await setUserContext(userId);
    ...
    const deleted = await database
        .delete(groupPosts)
        .where(and(eq(groupPosts.groupId, groupId), eq(groupPosts.postId, postId)))
        .returning();
    if (deleted.length === 0) {
    ...
    await database
        ...
        .where(and(eq(groupReactions.groupId, groupId), eq(groupReactions.postId, postId)));
    return { success: true };
}
// MODERATION: Remove member from group (owners/admins only)
export async function removeMemberFromGroup(userId, groupId, targetUserId) {
    const database = db;
    await setUserContext(userId);
    // Check if user is owner or admin
    const membership = await database
...
export async function getGroupProfile(userId, groupId) {
    const database = db;
    await setUserContext(userId);
...
export async function joinGroup(userId, groupId, inviteCode) {
    const database = db;
    await setUserContext(userId);
    ...
    if (!group[0].isPublic) {
        ...
        const invite = await database
            .select()
            .from(groupInvites)
            .where(and(eq(groupInvites.code, inviteCode), eq(groupInvites.groupId, groupId), gte(groupInvites.expiresAt, new Date())))
            .limit(1);
        if (!invite[0]) {
...
export async function createGroupInvite(userId, groupId, invitedEmail) {
    const database = db;
...
export async function acceptInvite(userId, inviteCode) {
    ...
    const invite = await database
        .select()
        .from(groupInvites)
        .where(and(eq(groupInvites.code, inviteCode), gte(groupInvites.expiresAt, new Date())))
        .limit(1);
    if (!invite[0]) {
        throw new Error("Invalid or expired invite code");
    }
    ...
    const existingMember = await database
        .select()
        .from(groupMembers)
        .where(and(eq(groupMembers.groupId, invite[0].groupId), eq(groupMembers.userId, userId)))
        .limit(1);
    if (existingMember[0]) {
        throw new Error("Already a member of this group");
    }
...
export async function createPost(userId, params) {
    ...
    const groupPostPromises = params.groupIds.map(groupId => database
        .insert(groupPosts)
        .values({
        groupId,
        postId: post[0].id,
...
export async function getGroupFeed(userId, groupId, options = {}) {
    const database = db;
    const { after, limit = 30 } = options;
    await setUserContext(userId);
    // Build where conditions
    const whereConditions = after
        ? and(eq(groupPosts.groupId, groupId), gte(groupPosts.createdAt, new Date(after)))
...
export async function toggleReaction(userId, params) {
    const database = db;
    await setUserContext(userId);
    ...
    const existing = await database
        .select()
        .from(groupReactions)
        .where(and(eq(groupReactions.groupId, params.groupId), eq(groupReactions.postId, params.postId), eq(groupReactions.userId, userId), eq(groupReactions.emoji, params.emoji)))
        .limit(1);
    if (existing[0]) {
        // Remove reaction
        await database
...
export async function getReactionSummary(userId, groupId, postId) {
    const database = db;
    await setUserContext(userId);
    const reactions = await database
        ...
        count: sql `count(*)`,
        userReacted: sql `bool_or(${groupReactions.userId} = ${userId})`,
    })
        .from(groupReactions)
        .where(and(eq(groupReactions.groupId, groupId), eq(groupReactions.postId, postId)))
...
// RSVP TO EVENT POST
export async function upsertRsvp(userId, params) {
    const database = db;
    await setUserContext(userId);
    ...
    const existing = await database
        ...
        .where(and(eq(groupEventRsvps.groupId, params.groupId), eq(groupEventRsvps.postId, params.postId), eq(groupEventRsvps.userId, userId)))
        .limit(1);
    if (existing[0]) {
...
// REMOVE RSVP
export async function removeRsvp(userId, groupId, postId) {
    const database = db;
    await setUserContext(userId);
    await database
        .delete(groupEventRsvps)
        .where(and(eq(groupEventRsvps.groupId, groupId), eq(groupEventRsvps.postId, postId), eq(groupEventRsvps.userId, userId)));
    return { action: "removed" };
}
// GET RSVPS FOR POST
export async function getPostRsvps(userId, groupId, postId) {
    const database = db;
    await setUserContext(userId);
    const rsvps = await database
        ...
        .where(and(eq(groupEventRsvps.groupId, groupId), eq(groupEventRsvps.postId, postId)))
...
export async function updateGroup(userId, groupId, updates) {
    ...
    const membership = await database
        .select()
        .from(groupMembers)
        .where(and(eq(groupMembers.groupId, groupId), eq(groupMembers.userId, userId), eq(groupMembers.role, "owner")));
...
export async function addMemberToGroup(userId, groupId, memberUserId, role = "member") {
    const database = db;
    await setUserContext(userId);
    ...
    const membership = await database
        .select()
        .from(groupMembers)
        .where(and(eq(groupMembers.groupId, groupId), eq(groupMembers.userId, userId), sql `${groupMembers.role} IN ('owner', 'admin')`));
    if (membership.length === 0) {
