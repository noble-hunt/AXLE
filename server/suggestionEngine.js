 ...
 */
export function computeDailySuggestion(context) {
    const rationale = [];
...
function getYesterdayWorkout(workouts) {
    const yesterday = subDays(new Date(), 1);
    return workouts.find(workout => differenceInDays(new Date(), workout.createdAt) === 1);
}
function isLowerBodyFocused(workout) {
    const lowerBodyCategories = [Category.POWERLIFTING];
    if (lowerBodyCategories.includes(workout.category))
        return true;
...
function analyzeWeeklyPatterns(workouts) {
...
function findLeastRecentCategory(workouts) {
    const categories = getAllCategories();
    const recent14Days = workouts
        .filter(w => differenceInDays(new Date(), w.createdAt) <= 14);
    // Find category not done in last 14 days
    const unusedCategories = categories.filter(cat => !recent14Days.some(w => w.category === cat));
...
function shouldPreferCategory(category, yesterdayWorkout) {
    if (!yesterdayWorkout)
        return true;
    // Don't do same category as yesterday
    if (category === yesterdayWorkout.category)
        return false;
    // Don't do lower body if yesterday was lower body
    if (isLowerBodyFocused(yesterdayWorkout) && category === Category.POWERLIFTING)
        return false;
    return true;
}
function analyzeHealthMetrics(report) {
...
function adjustDurationBasedOnHistory(workouts, intensity) {
