...
function normalizeOlympicWorkout(workout) {
    ...
    if (!normalized.name)
        normalized.name = "Olympic Training";
    if (!normalized.description)
        normalized.description = "Olympic weightlifting technical session";
    if (!normalized.equipment_needed)
        normalized.equipment_needed = ["barbell", "platform", "squat_rack"];
...
export async function generateOlympicWorkout(request) {
    try {
        // Build user prompt
        const userPrompt = `
