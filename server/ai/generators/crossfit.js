...
function normalizeWorkout(workout) {
    ...
    if (!normalized.name)
        normalized.name = "CrossFit Workout";
    if (!normalized.description)
        normalized.description = "High-intensity functional fitness workout";
    if (!normalized.equipment_needed)
        normalized.equipment_needed = ["barbell", "pull_up_bar"];
...
export async function generateCrossFitWorkout(request) {
    try {
        // Build user prompt
        const userPrompt = `
