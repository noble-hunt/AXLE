...
function applyRecoveryLogic(request) {
    ...
    if (health) {
        let cautionFlags = 0;
        if (health.hrv && health.hrv < 30)
            cautionFlags++;
        if (health.resting_hr && health.resting_hr > 80)
            cautionFlags++;
        if (health.sleep_score && health.sleep_score < 60)
            cautionFlags++;
    ...
    if (yesterday?.type === 'heavy_legs' ||
        (yesterday?.movements && yesterday.movements.some(m => ['squat', 'deadlift', 'lunge'].some(pattern => m.toLowerCase().includes(pattern))))) {
        if (!adjusted.context)
            adjusted.context = {};
        adjusted.context.constraints = [
            ...(adjusted.context.constraints || []),
...
function extractExplainableFields(workout) {
    const estimated_rpe = workout.intensity_1_to_10;
    const timecaps = [];
    const equipment_list = workout.equipment_needed || [];
    const hazards = [];
