...
export function upgradeIntensity(workout, equipment, readiness) {
    const hasLoad = (equipment || []).some(e => /(barbell|dumbbell|kettlebell)/i.test(e));
    let rot = 0;
    for (const b of workout.blocks) {
        if (['warmup', 'cooldown'].includes(b.kind))
            continue;
        ...
        if (/(Every\s*3:00|E3:00)/i.test(b.title))
            b.title = b.title.replace(/(Every\s*3:00|E3:00)/i, 'E2:00');
        if (/EMOM\s*10\b/i.test(b.title))
            b.title = b.title.replace(/EMOM\s*10\b/i, 'EMOM 14');
        // 3) Increase volume
        for (const it of (b.items || [])) {
            if (typeof it.scheme?.reps === 'number')
                it.scheme.reps = Math.round(it.scheme.reps * 1.25); // was 1.15
            if (/RIR\s*2-3/i.test(it.scheme?.rpe || ''))
                it.scheme.rpe = 'RIR 1–2';
        }
    }
...
const createPromptTemplate = (request) => {
...
${recentPRs.slice(0, 3).map((pr) => `- ${pr.exercise}: ${pr.weight ? `${pr.weight} ${pr.unit || 'lbs'}` : ''} ${pr.reps ? `x${pr.reps} reps` : ''} (${new Date(pr.date).toLocaleDateString()})`).join('\n')}`
        : "No recent PRs available.";
    // Format last 3 workouts for variety context
    const workoutContext = lastWorkouts.length > 0
        ? `Last 3 Workouts:
${lastWorkouts.slice(0, 3).map((workout) => `- ${workout.name} (${workout.category}): ${workout.duration}min, ${workout.intensity}/10 intensity, Exercises: ${workout.exercises.join(', ')} - ${new Date(workout.date).toLocaleDateString()}`).join('\n')}`
...
export function convertPremiumToGenerated(premium) {
    const sets = [];
    // Warm-up
...
async function generateWithOpenAI(request) {
    ...
    if (normalizedEquipment.length > 0) {
        const filtered = MOVEMENTS.filter(m => m.equipment.some(e => normalizedEquipment.includes(e)) ||
...
7. Keep coaching cues to 1-2 sentences max with embedded scaling suggestions`;
    ...
    try {
        ...
        const headers = aiResponse.sets?.filter((s) => s.is_header) || [];
        ...
        const cardioSets = aiResponse.sets?.filter((s) => {
            const name = (s.exercise || '').toLowerCase();
            return cardioNames.some(c => name.includes(c));
        }) || [];
        if (cardioSets.length > 0) {
            console.log('[WG] DEBUG: Cardio exercises from OpenAI:', cardioSets.slice(0, 5).map((s) => ({
                exercise: s.exercise,
    ...
    const validMovementNames = new Set(availableMovements.map(m => m.name.toLowerCase()));
    // Validate and map exercises to ensure they exist in the movement library
    const validatedSets = (aiResponse.sets || []).map((set, index) => {
        ...
        if (exerciseLower.length >= 4) {
            const fuzzyMatch = availableMovements.find(m => m.name.toLowerCase().includes(exerciseLower) ||
    ...
    const nonHeaderSets = validatedSets.filter((s) => !s.is_header);
...
export async function generateWorkout(request) {
...
async function generateSimpleFallback(request) {
    if (!openai)
        throw new Error('No OpenAI API available');
    ...
    const workoutData = {
        ...
        duration: request.duration,
        intensity: request.intensity,
        sets: aiResponse.sets?.map((set, index) => ({
...
export function generateMockWorkout(request, reason) {
...
12:00 time cap!`,
    ...
    const template = workoutTemplates[request.category] || workoutTemplates[Category.STRENGTH];
    return {
        name: template.name,
        category: request.category,
        ...
        duration: request.duration,
        intensity: request.intensity,
        sets: template.exercises.map((exercise, index) => ({
