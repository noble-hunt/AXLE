...
async function createEventPost(authorId: string) {
...
async function seedPosts(groupsData: any[]) {
...
async function seedReactions(groupsData: any[]) {
  ...
  for (const group of groupsData) {
    ...
    for (const post of groupPostsData) {
      ...
      for (let i = 0; i < numReactions; i++) {
        ...
        if (randomMember.userId === post.authorId) continue;
