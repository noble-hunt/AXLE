 ...
 */
async function syncHealthDataIfNeeded(userId, today) {
    // Check if user has connected health devices
    const connectedDevices = await db
        .select()
        .from(wearableConnections)
        .where(and(eq(wearableConnections.userId, userId), eq(wearableConnections.connected, true)));
    ...
    for (const device of connectedDevices) {
        ...
        `);
            ...
            await db
                ...
                .where(and(eq(wearableConnections.userId, userId), eq(wearableConnections.provider, device.provider)));
        ...
        for (const { userId } of activeUsers) {
            try {
                ...
                const existingSuggestion = await db
                    ...
                    .where(and(eq(suggestedWorkouts.userId, userId), eq(suggestedWorkouts.date, today)))
                    .limit(1);
 ...
 */
export async function getTodaySuggestionsCount() {
 ...
 * Start the cron job
 */
export function startSuggestionsCron() {
