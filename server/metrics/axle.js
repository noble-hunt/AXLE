...
export async function computeAxleScores({ userId, dateISO, metrics }) {
    ...
    const totalActiveMins = recent.reduce((s, w) => s + (w.total_active_minutes ?? 0), 0);
    const avgIntensity = recent.length ? (recent.reduce((s, w) => s + (w.intensity ?? 5), 0) / recent.length) : 5; // 1-10 scale
    ...
    const vitality = 0.30 * (sleepQuality / 100) +
        0.25 * activity01 +
        0.25 * recovery01 +
    ...
    const performance = 0.50 * recovery01 +
    ...
    for (const w of last28) {
        const sys = w.energy_system ?? "aerobic";
