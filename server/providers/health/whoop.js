...
async function authedFetch(userId, input, init) {
    let tok = await getDecryptedTokens(userId, "Whoop");
    if (!tok)
        throw new Error("No WHOOP token on file");
...
export class WhoopHealthProvider {
    constructor() {
        this.id = "Whoop";
    }
    hasConfig() {
    ...
    async authStart(userId) {
        ...
        if (!hasConfigEnv())
            throw new Error("WHOOP not configured");
    ...
    async fetchLatest(userId) {
        ...
        try {
            ...
            try {
                const { response: cyclesRes, tokenRefreshed: refreshed1 } = await authedFetch(userId, `${WHOOP_API}/cycle?limit=5&start=${start.toISOString()}&end=${end.toISOString()}`);
            ...
            try {
                const { response: recRes, tokenRefreshed: refreshed2 } = await authedFetch(userId, `${WHOOP_API}/recovery?limit=5&start=${start.toISOString()}&end=${end.toISOString()}`);
            ...
            // 3) Sleep (collection)
            try {
                const { response: sleepRes, tokenRefreshed: refreshed3 } = await authedFetch(userId, `${WHOOP_API}/sleep?limit=5&start=${start.toISOString()}&end=${end.toISOString()}`);
            ...
            try {
                const { response: wRes, tokenRefreshed: refreshed4 } = await authedFetch(userId, `${WHOOP_API}/workout?limit=1&start=${start.toISOString()}&end=${end.toISOString()}`);
