...
export class GarminHealthProvider {
    constructor() {
        this.id = "Garmin";
    }
    hasConfig() {
    ...
    async authStart(userId) {
        if (!garmin.clientId || !garmin.redirectUrl)
    ...
    async authCallback(params, userId) {
        ...
        if (decoded.p !== "garmin" || decoded.u !== userId)
    ...
    async fetchLatest(userId) {
        ...
        if (!uidRes.ok)
            throw new Error(`Garmin user id failed: ${uidRes.status}`);
        ...
        if (!dailiesRes.ok)
            throw new Error(`Garmin dailies failed: ${dailiesRes.status}`);
    ...
    async ensureGarminAccessToken(tokens, userId) {
        ...
        if (!res.ok)
            throw new Error(`Garmin refresh failed: ${res.status}`);
