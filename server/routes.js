...
export async function registerRoutes(app) {
    ...
    app.get("/api/dev/workouts/library", (req, res) => {
        try {
            ...
            if (type)
                filter.type = type;
            if (energySystem)
                filter.energySystem = energySystem;
            if (movementPattern)
                filter.movementPattern = movementPattern;
            if (experience)
                filter.experience = experience;
            if (maxDuration)
                filter.maxDuration = Number(maxDuration);
    ...
    app.post("/api/dev/workouts/generate", async (req, res) => {
        ...
        try {
            ...
            const history = []; // Empty history for dev testing
            const progressionStates = []; // No progression tracking for dev
            // Generate workout plan
            const workoutPlan = generateWorkoutPlan(workoutRequest, history, progressionStates, biometrics);
    ...
    app.get("/api/dev/workouts/simulate", async (req, res) => {
        ...
        try {
            ...
            const currentProfile = profiles[profileLevel] || profiles.intermediate;
            ...
            for (let day = 0; day < numDays; day++) {
                ...
                // Generate workout plan
                const workoutPlan = generateWorkoutPlan(workoutRequest, workoutHistory.slice(-7), // Last 7 workouts for context
    ...
    app.post("/api/workouts/preview", async (req, res) => {
        ...
        try {
            ...
            const generatedWorkout = await generateWorkout(generatorRequest);
    ...
    app.post("/api/workouts/generate", async (req, res) => {
        ...
        try {
            ...
            const workout = await generateWorkout(reqBody);
    ...
    app.get("/api/pr-stats", requireAuth, async (req, res) => {
        try {
            ...
            prs.forEach(pr => {
                if (!movementHistory[pr.movement])
                    movementHistory[pr.movement] = [];
    ...
    app.get("/api/achievement-stats", requireAuth, async (req, res) => {
        try {
            ...
            const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
            const recentUnlocks = unlockedAchievements.filter(a => a.updatedAt && new Date(a.updatedAt) >= thirtyDaysAgo);
    ...
    app.get("/api/health/supabase", async (req, res) => {
        try {
            const clientEnvPresent = Boolean(process.env.VITE_SUPABASE_URL && process.env.VITE_SUPABASE_ANON_KEY);
            const serverEnvPresent = Boolean(process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY);
            let canQuery = false;
            if (serverEnvPresent) {
                try {
    ...
    app.post("/api/workouts/feedback", requireAuth, async (req, res) => {
        try {
            const authReq = req;
            const { workoutId, generationId, intensityFeedback, difficultyRating, rpe, completionPercentage, overallSatisfaction, comments, sessionDuration } = req.body;
    ...
    app.get("/api/reports", requireAuth, async (req, res) => {
        try {
            ...
            const limit = req.query.limit ? parseInt(req.query.limit) : 50; // Default limit of 50
