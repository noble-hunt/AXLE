...
function normalizeToStyle(raw) {
    ...
    // Direct match
    if (SUPPORTED_STYLES.includes(lower))
        return lower;
    // Common aliases
    const aliases = {
        'cf': 'crossfit',
    ...
    if (lower.includes('bodybuilding'))
        return 'bb_full_body';
...
export function createUserHash(userId) {
...
export function createGenerationSeed(userId, focus, nonce) {
