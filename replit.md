...
- **Comprehensive Workout Stats Page**: Added `/stats` route with detailed analytics including:
  - Time-range filtering (Last Week/Month/Year)
  - Key metrics dashboard (total workouts, time, completion rate, average duration)
  - Streak tracking (current streak, longest streak, new PRs)
  - Interactive visualizations: Workout Activity area chart, Category Distribution pie chart, Intensity Levels bar chart (using Recharts library)
  - Personalized insights cards (best workout day, active days percentage, consistency metrics)
  - Robust data validation with `parseSafeDate()` and `getSafeDuration()` helpers to handle invalid/missing data
  - Comprehensive error handling (loading, error, empty states)
  - Accessible from workout history page via "View More Stats" button
- **Enhanced Filter Modal UI**: Improved segmented control spacing with `gap-1` class for better button separation in workout history filters (All/Completed/Pending)
- **Added GET /api/prs Endpoint**: REST API endpoint for fetching personal records, supporting the stats page data requirements
- **Verified Completed Workout Tracking**: Confirmed that completion rate and completed workout counts properly display in AXLE Reports section using existing server/services/reportGenerator.ts logic
...
## Recent Changes (November 16, 2025)
- **Fixed Workout Completion Critical Bug**: Resolved production-blocking issue where users couldn't complete workouts. The `workout_feedback` table was defined in the schema but missing from the database, causing "Could not find the table 'public.workout_feedback' in the schema cache" errors. Created the table with proper schema: id (UUID), workout_id (UUID FK), user_id (UUID FK), perceived_intensity (SMALLINT 1-10 RPE), notes (TEXT), created_at (TIMESTAMP), and unique constraint ensuring one feedback per user per workout. Workout completion now saves correctly to the database, enabling proper data flow to Reports, Analytics, and Achievement systems. Test user credentials: athlete@axlapp.com / password123!
